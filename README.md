...
**複数案件を並列で管理する** フリーランスイラストレーター向けの、PC 専用スケジュール管理アプリ。
水平ガントチャートで案件 × 工程 × 日付を一画面に並べ、完了案件が増えるほど **案件タイプ別の平均工程時間** が見える化され、見積もりの参考にできます。

![ダッシュボード全景](docs/screenshots/image.png)

![カードビュー](docs/screenshots/image-1.png)

> イラストレーターである作者が自分の案件管理用に作ったツールです。案件のスケジュール管理をしながら作業時間を分析したいと考え、実績を蓄積して可視化できるアプリが欲しくて開発しました。
> ご自由にお使いください（広告なし・無料）。個人利用 / ソースコードの閲覧は OK、再配布や販売はご遠慮ください（[ライセンス](#ライセンス--利用条件) を参照）。

### こんな人向け

- 複数のクライアント案件を並行して進めるイラストレーター
- 工程ごとにかかる時間を把握して短縮を目指したい人、見積もりの参考にしたい人
- スケジュール管理、事務タスク、作業レポートを一元管理したい人
...
> **開発について**
> 本アプリのコード実装にはAIコーディングアシスタント（Anthropic の Claude Code）を活用しています。
> アプリ自体に画像生成AIや生成AI機能は含まれていません。

---

## 特徴1: 完了案件が増えるほど見積もり精度が上がる

ポモドーロタイマーで作業を計測すると、その時間が **今日の作業記録** と **工程の実績時間** に自動入力されます。データが蓄積されると **案件タイプごとの工程平均作業時間** が自動で算出され、複数案件をこなした **自分自身の傾向** がそのまま可視化されます。
どこに時間がかかっているか分析したり、新規案件見積もりの参考にすることができます。
...
![ポモドーロタイマーとレポート画面](docs/screenshots/app_features.png)


## 特徴2: 自由なカスタマイズ

テーマカラーはプリセット（paper / mint / lavender / ink / sakura）または
オリジナルテーマから選択可能。
その他、以下の項目で色やアイコンを変更できます。

- 案件タイプの色
- 工程の色
- ダッシュボードのグリーティングアイコン

![テーマイメージ](docs/screenshots/theme.png)

![グリーティングアイコン](docs/screenshots/g-icon.png)
...
![Releases ページの場所](docs/screenshots/github_ss_1.png)

![Releases ページの場所](docs/screenshots/github_ss_2.png)

### Windows のセキュリティ警告について

インストール時に **「Windows によって PC が保護されました」** という青い画面が表示されます。

![警告](docs/screenshots/alart_1.png)
...
![警告→詳細情報→実行の手順](docs/screenshots/alart_2.png)
...
- 水平ガントチャート（タイムライン）
- ドラッグで作業日を移動・伸縮
- 案件の締め切りとは別に、工程ごとに提出日を設定可能
- 案件タイプ別の工程セットテンプレート
- 一日ごとの簡易メモ記入や提出日のセットが可能（ガントチャートのみ）
- 案件ごとに事務タスクのチェックリストをセット可能（カードビューのみ）
...
### レポート
- 期間別（月 / 年 / 累計）の集計
- 案件タイプ別の工程平均時間 / 工程別の累計時間（同じカードでタブ切替）
- 案件タイプ別配分（ドーナツ）
- 期間内に完了した案件一覧
- ダッシュボードの日別作業時間グラフは **週 / 月 / 年** で粒度を切替可能

### カスタマイズ
- テーマ（paper / mint / lavender / ink / sakura + オリジナルテーマ）
- 案件タイプ・工程の色相を個別 + 全体の彩度・明度を一括調整
- 平日 / 休日の作業時間設定
- グリーティングアイコン: 任意の画像をアップロードして好みのアイコンに変更可能
- 文字サイズ切替（小 / 中 / 大）: 老眼やディスプレイサイズに合わせて全体を拡大表示
...
- 効果音 (`assets/alart*.mp3`): フリー素材（効果音ラボ）
